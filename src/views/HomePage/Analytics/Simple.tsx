...
const Simple = () => {
  ...
  const getReport = async (file_name: any) => {
    ...
    try {
      const result = await generatorAnalytics(file_name)
  ...
  const getHistory = async () => {
    const result = await getAnalyticsHistory()
    let res: Array<AnalyticsHistoryChildren> = result.data
  ...
  const getHistoryContent = async (id: number) => {
    ...
    try {
      const result = await getAnalyticsById(id)
      const res = result.data
      setGeneratedContent(JSON.parse(res.content))
      dealWithDataCharts(JSON.parse(res.content))
      setContentStatus(true)
    ...
    <>
      <div className={`flex flex-col`}>
        <div className="flex flex-row mt-14" style={{ marginLeft: pxToVw(29) }}>
          ...
          >
            {t('Simple Analytics Tool')}
          </div>
        </div>
        <div
          className={`text-[#545B65] mt-4 ml-29 text-14`}
          style={{ fontFamily: 'PingFang SC Light' }}
        >
          {t(
            'Get your data all set with our simple analytics tool. Just a click away.'
          )}
        </div>
      </div>
      {!loading ? (
        <>
          <div
            className={`bg-white rounded-8 mt-14 w-1389 ml-29`}
            style={{
            ...
            {!contentStatus ? (
              <div className={`flex justify-between items-start`}>
                ...
                <div className={`h-821 overflow-y-auto`}>
                  <div
                    className={`w-682 h-750 flex flex-col items-center justify-center`}
                  >
                    <Icon
                      name={'generate'}
                    ...
                    >
                      {t(
                        'Upload your sales or user data (preferably a .csv file) of your business and generate aggregation charts, correlation analysis charts and interpretations.'
                      )}
                    </p>
                  </div>
                </div>
                ...
                <div className={`pl-30`}>
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: generatedContent.positive_report,
                    }}
                  ></div>
                  ...
                  <div className={`px-69 pt-40`}>
                    <Select
                      style={{ width: pxToVw(200) }}
                      defaultValue={positiveOptions[0].value}
                  ...
                  <div>
                    <Plot
                      data={positiveChartsData}
                      layout={positiveChartsLayout}
                    />
                  </div>
                  ...
                  <div
                    className={`px-69`}
                    dangerouslySetInnerHTML={{
                      __html: generatedContent.negative_report,
                    }}
                  ></div>
                  ...
                  <div className={`px-69 pt-40`}>
                    <Select
                      style={{ width: pxToVw(200) }}
                      defaultValue={negativeOptions[0].value}
                  ...
                  <div>
                    <Plot
                      data={negativeChartsData}
                      layout={negativeChartsLayout}
                    />
                  </div>
                </div>
              </div>
        ...
        <>
          <div
            className={`w-1372 h-809 bg-white rounded-8 mt-28 flex items-center justify-center`}
          >
            <div className={`flex flex-col items-center`}>
              ...
              <div
                className={`text-14 text-[#C4C4C4] w-432 text-center leading-16`}
              >
                {t('Please wait for a few minutes. Do not refresh the page.')}
              </div>
            </div>
          </div>
        </>
