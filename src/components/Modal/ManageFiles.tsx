...
const ManageFiles: FC<Props> = (props) => {
  ...
  const beforeUpload: (
    ...
    try {
      ...
      props.onUpload({ name: file.name, url: result.data })
    } catch (e) {
      console.error(e)
    }
            ...
            >
              <div className="h-294 flex justify-center items-center flex-col">
                ...
                <div className={`mt-2 text-black opacity-60 text-10`}>
                  {t('Upload more files to knowledge base')}
                </div>
                <div className={`mt-2 text-black opacity-60 text-10`}>
                  {t('Drop files here')}
                </div>
            ...
            <div className="w-full h-336 bg-[#FFFFFF] flex flex-wrap items-start content-start rounded-4 border border-[#DBDBDB] border-solid py-12 px-10">
              {files.map((f) => {
                return (
                  <div
                    key={f.url}
                    className="flex flex-col items-center relative mr-4"
                  >
                    <div
                      onClick={() => {
                        removeFile(f)
                      }}
                      className={`w-10 h-10 flex items-center justify-center bg-[#FA4848] rounded-full cursor-pointer absolute -top-3 right-1`}
                    >
                      <Icon
                        name={'remove'}
                        style={{ width: pxToVw(4), height: pxToVw(4) }}
                      />
                    </div>
                    <Icon
                      name="pdf"
                      style={{ width: pxToVw(45), height: pxToVw(45) }}
                    />
                    <div className="text-6 text-[#818181] mt-5 w-30 text-center truncate">
                      {f.name}
                    </div>
                  </div>
                )
